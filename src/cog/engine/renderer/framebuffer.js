...
let 
  ext = gl.extensions.WEBGL_draw_buffers,
  attachment = [
...
let checkFbo = () => {
  ...
  switch(result) {
    ...
    case gl.FRAMEBUFFER_INCOMPLETE_ATTACHMENT: console.warn("FrameBuffer incomplete - attachment"); break;
    case gl.FRAMEBUFFER_INCOMPLETE_DIMENSIONS: console.warn("FrameBuffer incomplete - dimensions"); break;
    case gl.FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: console.warn("FrameBuffer incomplete - missing attachment"); break;
    case gl.FRAMEBUFFER_UNSUPPORTED: console.warn("FrameBuffer incomplete - unsupported"); break;
    default: console.warn("FrameBuffer incomplete - unknown error (" + result + ")");
  }
};
...
class FrameBuffer {
  constructor(width = 256, height = 256) {
    ...
    if(Object.keys(gl.extensions).some( e => e.indexOf("_depth_texture") !== -1 )) {
      this.depthBuffer = new RTDepthTexture(this.width, this.height);
      ...
      this.depthBuffer = new RTDepth(this.width, this.height);
  ...
  addRenderTarget(index) {
    ...
    gl.bindFramebuffer(gl.FRAMEBUFFER, this.id); gl.inc();
    gl.framebufferTexture2D(gl.FRAMEBUFFER, attachment[index], gl.TEXTURE_2D, this.renderTarget[index].id, 0); gl.inc();
    checkFbo();
  }
  ...
  begin(buffers) {
    ...
    ext.drawBuffersWEBGL(buffers.map(b => attachment[b]));
  }
